...
def make_map(significant_places, corpus_df, basemap, marker_size, center=None, zoom=None):
    ...
    def create_map():
        ...
        logging.info(f"Creating map with {len(significant_places_clean)} places")
            ...
            options={
                ...
                'iconCreateFunction': """
                    function(cluster) {
                        ...
                        var width = Math.min(Math.max(Math.sqrt(count) * 10, 30), 80);
        ...
        for _, place in significant_places_clean.iterrows():
            try:
                ...
                logging.error(f"Error adding marker: {e}")
                logging.error(f"Problem place data: {place}")

        logging.info(f"Added {markers_added} markers to the map")
