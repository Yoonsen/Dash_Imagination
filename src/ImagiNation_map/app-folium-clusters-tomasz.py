...
def make_map(significant_places, corpus_df, basemap, marker_size, center=None, zoom=None):
    ...
    def create_map():
            ...
            options={
                ...
                'iconCreateFunction': """
                    function(cluster) {
                        ...
                        var size = Math.sqrt(count) * 3;
                        size = Math.min(Math.max(size, 30), 60);
                        
                        return L.divIcon({
                            html: '<div style="width: ' + size + 'px; height: ' + size + 'px; ' +
                                  'border-radius: 50%; background-color: rgba(70, 130, 180, 0.7); ' +
                                  'display: flex; align-items: center; justify-content: center; ' +
                                  'border: 3px solid rgba(70, 130, 180, 0.9); ' +
        ...
        for i in range(0, len(significant_places_clean), batch_size):
            batch = significant_places_clean.iloc[i:i+batch_size]
            
            for _, place in batch.iterrows():
                try:
                    feature_class = place['feature_class']
                    color = feature_colors.get(feature_class, 'gray')
                    
                    # Get place books for popup
                    place_books = corpus_df[corpus_df.dhlabid.isin(place['dhlabid'])]
                    popup_html = create_popup_html(place, place_books)
                    
                    # Calculate radius in meters based on frequency
                    # Adjust these values to get the desired size range
                    base_radius = 2000  # Base radius in meters
                    max_radius = 20000  # Maximum radius in meters
                    radius = min(base_radius + np.log(place['frekv']) * marker_size * 1000, max_radius)
                    
                    # Create Circle instead of CircleMarker
                    marker = folium.Circle(
                        location=[float(place['latitude']), float(place['longitude'])],
                        radius=radius,  # radius in meters
                        popup=folium.Popup(popup_html, max_width=500),
                        tooltip=f"{place['token']}: {place['frekv']} forekomster",
                        color=color,
                        fill=True,
                        fill_color=color,
                        fill_opacity=0.4,
                    ...
                    logging.error(f"Error creating marker: {str(e)}")
    
        marker_cluster.add_to(m)
        logging.info(f"Final marker count: {total_markers}")
